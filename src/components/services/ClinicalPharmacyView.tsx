    ...
    <div className="space-y-6">
      ...
      {patientAllergies.length > 0 && (
        <div className="p-4 rounded-xl bg-rose-950/40 border border-rose-500/40 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-rose-300 uppercase tracking-wider">Patient Allergy Watch</div>
            <div className="text-sm font-semibold text-white">
              Documented severe adverse reaction to: <span className="text-rose-200 underline">{patientAllergies.join(', ')}</span>.
          ...
          >
            ...
            {tab.badge && (
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${
                tab.badge === 'TIMED OK' ? 'bg-emerald-500/30 text-emerald-300' : 'bg-amber-500/30 text-amber-300'
              }`}>
                {tab.badge}
              </span>
            )}
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 backdrop-blur-md">
              ...
              <div className="space-y-3">
                {currentMeds.map((med, idx) => (
                  <div 
                    key={idx} 
                    className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60 hover:border-teal-500/40 transition-all flex items-start justify-between gap-4"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white text-base">{med.name}</span>
                        <span className="text-xs px-2 py-0.5 rounded-md bg-teal-500/20 text-teal-300 border border-teal-500/30 font-semibold">
                          {med.dose}
                        </span>
                      ...
                      <div className="text-xs text-slate-400 flex items-center gap-3">
                        <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-slate-500" /> Frequency: <strong className="text-slate-200">{med.frequency}</strong></span>
                        <span>• Status: <span className="text-emerald-400 font-semibold">Active Inpatient</span></span>
                      </div>
                    </div>

                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-lg">
              ...
              <form onSubmit={handleAddMed} className="mt-6 pt-5 border-t border-slate-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  <input
                    ...
                    className="sm:col-span-2 px-3 py-2 rounded-xl bg-slate-800/80 border border-slate-700 text-white text-xs focus:outline-none focus:border-teal-500"
                  />
                  <input
                    type="text"
                    ...
                    className="px-3 py-2 rounded-xl bg-slate-800/80 border border-slate-700 text-white text-xs focus:outline-none focus:border-teal-500"
                  />
                  <select
                    value={newMedFreq}
                ...
                <button
                  type="submit"
                  className="mt-3 px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold shadow-md transition-all flex items-center gap-1.5"
                >
          ...
          <div className="space-y-4">
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 backdrop-blur-md">
              ...
              <div className="p-3 rounded-xl bg-teal-500/10 border border-teal-500/20 mb-3">
                <div className="text-xs font-bold text-teal-300">{clinicalPharmacist.name}</div>
                <div className="text-[11px] text-slate-400">{clinicalPharmacist.desig}</div>
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 backdrop-blur-md">
              <div className="flex items-center gap-2 text-teal-400 mb-2">
                ...
                <span className="text-xs font-bold text-white uppercase">NABH Medication KPI</span>
              </div>
      ...
      {activeTab === 'sap' && (
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl bg-slate-800/60 border border-slate-700">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-2xl">
                ...
                <strong className="text-teal-300"> 60 minutes prior to surgical skin incision</strong>. Vancomycin (if MRSA colonized/allergic) requires 120 minutes.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/60">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Primary Regimen</span>
              <div className="text-sm font-bold text-white mt-1">Cefuroxime 1.5g IV</div>
              <div className="text-xs text-slate-400 mt-1">Single dose over 3-5 mins at anesthesia induction. Redose at 4 hours if surgery prolonged.</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/60">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">MRSA / Penicillin Allergy</span>
              <div className="text-sm font-bold text-white mt-1">Vancomycin 1g IV + Gentamicin 5mg/kg</div>
              <div className="text-xs text-slate-400 mt-1">Infusion initiated 90-120 minutes prior to incision to prevent red-man syndrome.</div>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/60">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Post-Op Discontinuation</span>
              <div className="text-sm font-bold text-white mt-1">Stop within 24 hours</div>
              <div className="text-xs text-slate-400 mt-1">NABH stewardship rule: Uncomplicated spine instrumentation stops at 24h to prevent C. difficile.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'neuropathic' && (
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Stavya Spine Multimodal Neuropathic Analgesia</h3>
              <p className="text-xs text-slate-400">Targeted radicular dysesthesia, leg shooting pain, and opioid-sparing protocol</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <div className="text-xs font-bold text-teal-400 uppercase mb-1">Step 1: Baseline Anti-inflammatory</div>
              <div className="text-sm font-bold text-white">IV Paracetamol + Etoricoxib</div>
              <div className="text-xs text-slate-400 mt-2">Paracetamol 1000mg q6h + Etoricoxib 90mg OD (Gastro-protected with Pantoprazole 40mg).</div>
            </div>

            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <div className="text-xs font-bold text-cyan-400 uppercase mb-1">Step 2: Neuropathic Membrane Stabilizer</div>
              <div className="text-sm font-bold text-white">Pregabalin 75mg HS / Gabapentin</div>
              <div className="text-xs text-slate-400 mt-2">Blocks voltage-gated Ca2+ channels in dorsal root ganglion; relieves burning radiculopathy.</div>
            </div>

            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <div className="text-xs font-bold text-amber-400 uppercase mb-1">Step 3: Centrally Acting Muscle Relaxant</div>
              <div className="text-sm font-bold text-white">Thiocolchicoside 4mg / 8mg BD</div>
              <div className="text-xs text-slate-400 mt-2">GABA-A and glycine receptor agonist. Relieves paraspinal muscle spasm after pedicle screw placement.</div>
            </div>

            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <div className="text-xs font-bold text-rose-400 uppercase mb-1">Step 4: Breakthrough Rescue</div>
              <div className="text-sm font-bold text-white">IV Tramadol + Ondansetron</div>
              <div className="text-xs text-slate-400 mt-2">50mg slow infusion with anti-emetic. Used only if VAS pain score exceeds 6/10.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'anticoag' && (
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Spine DVT Chemoprophylaxis Timing Protocol</h3>
              <p className="text-xs text-slate-400">Balancing epidural hematoma risk with deep vein thrombosis prevention</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200 flex items-start gap-3">
            ...
            <div>
              <strong className="text-white block font-bold">Absolute Spine Hemostasis Requirement:</strong>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
              <div className="text-xs font-bold text-indigo-300 uppercase mb-2">Mechanical Prophylaxis (Immediate)</div>
            ...
            <div className="p-4 rounded-xl bg-slate-800/50 border border-slate-700">
              <div className="text-xs font-bold text-indigo-300 uppercase mb-2">Chemical Prophylaxis (Post-Op Day 1)</div>
              <ul className="text-xs text-slate-300 space-y-1.5 list-disc list-inside">
      ...
      {activeTab === 'counseling' && (
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-md space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <span className="text-xs font-bold text-teal-400 block mb-1">Pain Medicines (Analgesics)</span>
            ...
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <span className="text-xs font-bold text-teal-400 block mb-1">Nerve Medication (Pregabalin)</span>
              <p className="text-xs text-slate-300 leading-relaxed">
            ...
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700">
              <span className="text-xs font-bold text-teal-400 block mb-1">Stool Softener (Post-op Bowel)</span>
              <p className="text-xs text-slate-300 leading-relaxed">
