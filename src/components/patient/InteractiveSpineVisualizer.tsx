...
export const InteractiveSpineVisualizer: React.FC<InteractiveSpineVisualizerProps> = ({ 
  ...
  return (
    <div className="w-full bg-white rounded-2xl p-5 border border-blue-200 shadow-sm space-y-6 text-slate-900">
      {/* Title & View Switcher */}
        ...
        <div className="flex items-center p-1 rounded-xl bg-slate-100 border border-slate-200">
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition ${
              viewMode === 'anatomy' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Spinal Column
          </button>
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition ${
              viewMode === 'sagittal' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold transition ${
              viewMode === 'axial' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <div className="lg:col-span-5 p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
            ...
            <div className="space-y-1.5 max-h-[520px] overflow-y-auto pr-1">
              {VERTEBRAE.map(v => {
                ...
                let itemClass = 'border-slate-200 hover:border-blue-300 bg-white';
                if (isSelected) {
                  itemClass = 'border-blue-600 bg-blue-50/80 shadow-xs ring-1 ring-blue-500';
                } else if (isAffected) {
                  itemClass = 'border-amber-300 bg-amber-50/60 text-amber-900';
                }
                ...
                return (
                  <div
                    ...
                    className={`w-full p-2.5 rounded-xl border flex items-center justify-between cursor-pointer transition text-xs ${itemClass} group`}
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 group-hover:text-blue-600 transition flex items-center gap-1.5">
                          {v.label}
                          {isAffected && (
                            <span className="text-[9px] px-1.5 py-0.2 rounded bg-amber-100 text-amber-800 border border-amber-300 font-bold uppercase tracking-wider">
                              Target Site
                            </span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${
                        v.region === 'cervical' ? 'bg-sky-50 text-sky-700 border border-sky-200' :
                        v.region === 'thoracic' ? 'bg-indigo-50 text-indigo-700 border border-indigo-200' :
                        v.region === 'lumbar' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="p-5 rounded-2xl bg-white border border-blue-200 shadow-sm space-y-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200 font-mono font-bold">
                      Root: {activeVertebra.nerveRoot}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Anatomical segment · Region: <span className="capitalize font-semibold text-slate-700">{activeVertebra.region}</span>
                  </p>
                </div>

                {isPatientAffectedLevel ? (
                  <div className="p-2 rounded-xl bg-amber-50 border border-amber-300 text-amber-800 text-xs font-bold flex items-center gap-1.5 shrink-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                  ...
                  <div className="text-slate-500 text-[11px] pt-1 border-t border-slate-200">
                    {isPatientAffectedLevel ? (
                      ...
                      <span className="text-emerald-700 font-semibold">Normal 5/5 Grade</span>
                    )}
                  </div>
                </div>
                ...
                {/* Dermatome (Sensory) */}
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                  <span className="text-[10px] text-teal-700 uppercase tracking-wider font-bold block">
                  ...
                  <div className="text-slate-500 text-[11px] pt-1 border-t border-slate-200">
                    {activeVertebra.reflex && (
                      <span className="text-slate-600">
                        Reflex: <strong className="text-blue-700">{activeVertebra.reflex}</strong>
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                <span className="text-[10px] text-blue-700 uppercase tracking-wider font-bold flex items-center gap-1.5">
                ...
                {isPatientAffectedLevel && (
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200 text-[11px]">
                    ...
                    <span className="px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-700">
                      Spondylolisthesis: <strong className="text-blue-700">{patient.radiology.listhesisType}</strong>
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-blue-50/60 border border-blue-200 space-y-2">
                <span className="text-[10px] text-blue-800 uppercase tracking-wider font-bold flex items-center gap-1.5">
                ...
                {isPatientAffectedLevel ? (
                  <div className="space-y-1.5 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-700 pt-1">
                      <div className="p-2 rounded bg-white border border-blue-100 shadow-2xs">
      ...
      {viewMode === 'sagittal' && (
        <div className="p-6 rounded-2xl bg-white border border-blue-200 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Formula: PI = PT + SS | Target: Lumbar Lordosis (LL) within ±9° of Pelvic Incidence (PI)
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Pelvic Incidence (PI)</span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Pelvic Tilt (PT)</span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Sacral Slope (SS)</span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Lumbar Lordosis (LL)</span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">PI – LL Mismatch</span>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center">
              <span className="text-[10px] text-slate-500 uppercase font-bold">Sagittal Axis (SVA)</span>
      ...
      {viewMode === 'axial' && (
        <div className="p-6 rounded-2xl bg-white border border-blue-200 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <span className="text-xs font-mono text-blue-700 px-2.5 py-1 rounded bg-blue-50 border border-blue-200 font-bold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="p-6 rounded-xl bg-slate-50 border border-slate-200 flex flex-col items-center justify-center relative">
              <svg viewBox="0 0 300 240" className="w-64 h-56">
                ...
                <ellipse cx="150" cy="115" rx="75" ry="30" fill="#f8fafc" stroke="#64748b" strokeWidth="1.5" />
                ...
                <circle cx="150" cy="155" r="32" fill="#3b82f6" fillOpacity="0.15" stroke="#2563eb" strokeWidth="2" strokeDasharray="3,3" />
                ...
                <text x="150" y="235" textAnchor="middle" fill="#64748b" fontSize="10">Spinous Process (Posterior)</text>
              </svg>
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                <span className="text-[10px] text-slate-500 uppercase font-bold">Schizas Stenosis Grading</span>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200">
                <span className="text-[10px] text-slate-500 uppercase font-bold">Decompression Clearance Plan</span>
